...
class ntlm_auther:
    ...
    def proxy_ntlm_authentication(self, connection):
        ...
        tmp_client_head_obj.replace_param_value(
            "Proxy-Authorization", "NTLM " + NTLM_msg1
        )
        ...
        while (not connection.rserver_all_got) and (
            not connection.rserver_socket_closed
        ):
        ...
        tmp_client_head_obj = connection.client_head_obj.copy()
        tmp_client_head_obj.replace_param_value(
            "Proxy-Authorization", "NTLM " + NTLM_msg3
        )
    ...
    def build_env_dict(self, connection):
        ...
        if (
            connection.config["NTLM_AUTH"]["LM_HASHED_PW"]
            and connection.config["NTLM_AUTH"]["NT_HASHED_PW"]
        ):
