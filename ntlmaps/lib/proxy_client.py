...
class proxy_HTTP_Client:
    def __init__(self, client_socket, address, config):
        ...
        self.logger_auth = logger.Logger(
            "%s-%d.auth" % address, self.config["DEBUG"]["AUTH_DEBUG"]
        )
        ...
        while not self.stop_request:
            ...
            if not (self.rserver_buffer or self.client_buffer):
                ...
                if not self.rserver_socket_closed:
                    try:
                        select.select(
                            [self.rserver_socket.fileno(), self.client_socket.fileno()],
                            [],
                            [],
                            5.0,
                        )
    ...
    def run_rserver_loop(self):
        ...
        if not self.rserver_head_obj and not self.tunnel_mode:
            ...
            if (
                self.rserver_socket_closed
                and self.rserver_buffer
            ...
            if self.rserver_data_length:
                if self.rserver_all_got:
                    data = self.rserver_buffer[
                        : self.rserver_data_length - self.rserver_current_data_pos
                    ]
    ...
    def rollback_client_data(self):
        ...
        if self.client_sent_data:
            ...
            self.client_sent_data = ""
            self.logger.log(
                "Rollback Done. (Client's buffer - %d bytes)\n"
                % len(self.client_buffer)
            )
        ...
        if not self.rserver_all_got:
            try:
                self.rserver_data_length = int(
                    self.rserver_head_obj.get_param_values("Content-Length")[0]
                )
                self.logger.log(
                    "*** Server 'Content-Length' found to be %d.\n"
    ...
    def guess_client_data_length(self):
        ...
        if not self.client_head_obj.has_param(
            "Content-Length"
        ) and not self.client_head_obj.has_param("Transfer-Encode"):
        ...
        if not self.client_all_got:
            try:
                ...
                self.logger.log(
                    "*** Client 'Content-Length' found to be %s.\n"
    ...
    def check_stop_request(self):
        ...
        if self.rserver_socket_closed and not self.first_run:
            ...
            if self.rserver_head_obj:
                ...
                if (
                    not self.proxy_authorization_tried
                ) and self.rserver_head_obj.get_http_code() == "407":
                    self.stop_request = 0
                if (
                    not self.www_authorization_tried
    ...
    def exit(self):
        ...
        if (
            self.rserver_socket_closed
            and self.rserver_buffer
            and (not self.client_socket_closed)
        ):
            self.logger.log(
        ...
        if (
            self.client_socket_closed
            and self.client_buffer
            and (not self.rserver_socket_closed)
        ):
            self.logger.log(
